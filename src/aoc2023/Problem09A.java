...
//        var str = Problem09Input.INPUT03;
        ...
        System.out.println(
                str.lines()
                        .map(Problem09A::calculateNextValue)
                        .reduce(Integer::sum)
                        .orElse(0)
        );
    }

    private static Integer calculateNextValue(String s) {
        var seq = parse(s);
        var seqList = createSeqList(seq);
        return getNextValue(seqList);
    }
    ...
    private static int getNextValue(List<List<Integer>> seqList) {
        var value = 0;
        for (int i = seqList.size()-1; i >= 0 ; i--) {
            value += seqList.get(i).get(seqList.get(i).size()-1);
        }
        return value;
    }
